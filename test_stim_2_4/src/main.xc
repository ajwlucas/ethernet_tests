...
int test_4(out buffered port:32 p_mii_txd)
{
    unsigned int buf[1600/4];
    int n = 0;
    unsigned char dest[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    unsigned char src[] = {0x00, 0x12, 0x34, 0x56, 0x78, 0x90};
    unsigned short len = 0x002F;
    int j = 64;
    
    create_packet(dest, src, len, buf, 1600);
    
    printstrln("Running test 4");
    
    for (unsigned short i=0x002F; i < 0x05FF; i++)
    {
        set_length_ethertype(buf, i);
        mii_transmit(buf, j, p_mii_txd, 0);
    }
