...
int test_5(out buffered port:32 p_mii_txd)
{
    unsigned int buf[10000/4];
    int n = 0;
    unsigned char dest[] = {0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF};
    unsigned char src[] = {0x00, 0x12, 0x34, 0x56, 0x78, 0x90};
    unsigned short len = 0xF0F0;
    
    create_packet(dest, src, len, buf, 9367);
    
    printstrln("Running test 5");

    set_seq_num(buf, 0);
    mii_transmit(buf, 9367, p_mii_txd, 0);

    set_seq_num(buf, 1);
    mii_transmit(buf, 500, p_mii_txd, 0);
