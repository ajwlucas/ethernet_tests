...
void mii_transmit(unsigned int buf[], int length, out buffered port:32 p_mii_txd, int bad_crc, int bad_preamble)
{
    ...
    else if (bad_preamble == 1)
    {
        p_mii_txd <: 0x00000055;
        p_mii_txd <: 0xD5000000;
    } 
    else if (bad_preamble == 2)
    {
        p_mii_txd <: 0xFFFF7F55;
        p_mii_txd <: 0xD5FFFFFF;
    }
    else if (bad_preamble == 3)
    {
        p_mii_txd <: 0x55555555;
        p_mii_txd <: 0xD5AFA855;
    }
    else
    {
        return;
...
int test_7(out buffered port:32 p_mii_txd)
{
    ...
    set_seq_num(buf, 1);
    mii_transmit(buf, 500, p_mii_txd, 0, 1); 

    set_seq_num(buf, 2);
    mii_transmit(buf, 500, p_mii_txd, 0, 0);

    set_seq_num(buf, 3);
    mii_transmit(buf, 500, p_mii_txd, 0, 2); 

    set_seq_num(buf, 4);
    mii_transmit(buf, 500, p_mii_txd, 0, 0); 

    set_seq_num(buf, 5);
    mii_transmit(buf, 500, p_mii_txd, 0, 3); 
