...
int receiver(chanend rx)
{
    ...
    while (1)
    {
        ...
        if (get_seq_num(rxbuffer) == 2) expected_seq_num = 4;
        else expected_seq_num += 1;
...
int mac_rx_bad_align_test(chanend tx, chanend rx)
{
    return receiver(rx);
}
...
void runtests(chanend tx[], chanend rx[], int links)
{
    ...
    RUNTEST("Test #2.7 - Frames with alignment errors", mac_rx_bad_align_test(tx[0], rx[0]));
    printstrln("Complete");
}
