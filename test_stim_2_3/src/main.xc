...
// Untagged frame
int test_3a(out buffered port:32 p_mii_txd)
{
    ...
    create_packet(dest, src, etype, buf, 1600);
    
    printstrln("Running test 3a");
    
    for (int i=1514; i < 1550; i++)
    {
        set_seq_num(buf, i);
        mii_transmit(buf, i, p_mii_txd, 0);
    }
...
int test_3b(out buffered port:32 p_mii_txd)
{
...
int main(void)
{
    ...
    #ifdef TEST_3A
            test_3a(mii.p_mii_txd);
    #endif
    #ifdef TEST_3B
            test_3b(mii.p_mii_txd);
    #endif
        }
    #ifdef SIMULATION
        on stdcore[ETH_CORE_ID]:
